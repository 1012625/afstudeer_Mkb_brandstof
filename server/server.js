...
app.get("/overview", (req, res) => {
  db.all("SELECT id, liters, type, opties FROM klantinfo", (err, rows) => {
    if (err) {
      console.error("Error fetching overview:", err.message); // Log error message
      return res.status(500).json({ error: "Error fetching overview" });
    }
